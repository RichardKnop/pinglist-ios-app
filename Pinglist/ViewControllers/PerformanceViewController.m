...
- (void)viewDidLoad {
  ...
  self.btnTrend.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:14.0f];
  self.btnIncident.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:14.0f];

  self.btnIncident1Hour.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:13.0f];
  self.btnIncident24Hour.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:13.0f];
  self.btnIncident30Day.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:13.0f];
  self.btnIncident7Day.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:13.0f];
  self.btnIncidentAllTime.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:13.0f];
  ...
  self.btnAllTime.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:13.0f];
  self.btn1Hour.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:13.0f];
  self.btn24Hours.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:13.0f];
  self.btn7Days.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:13.0f];
  self.btn30Days.titleLabel.font =
      [UIFont fontWithName:@"Lato-Regular" size:13.0f];
  ...
  self.txtViewMessage.textContainerInset = UIEdgeInsetsMake(
      0, -self.txtViewMessage.textContainer.lineFragmentPadding, 0,
      -self.txtViewMessage.textContainer.lineFragmentPadding);
  self.txtViewResponse.textContainerInset = UIEdgeInsetsMake(
      0, -self.txtViewResponse.textContainer.lineFragmentPadding, 0,
      -self.txtViewResponse.textContainer.lineFragmentPadding);
...
- (void)initGraph {
  ...
  NSString *requestURL = [NSString
      stringWithFormat:
          @"/v1/alarms/%d/response-times?limit=100&order_by=timestamp desc",
          self.alarm.ID];
  ...
  [UIView animateWithDuration:.3f
      animations:^{
        self.btnSelectionbar.center = CGPointMake(
            self.btnAllTime.center.x, self.btnSelectionbar.center.y);
      }
      completion:^(BOOL finished) {
        [self.btnAllTime setTitleColor:GREEN_COLOR
...
- (void)refresh {
  NSString *requestURL =
      [NSString stringWithFormat:@"/v1/alarms/%d/incidents?order_by=id desc",
                                 self.alarm.ID];
...
- (void)resetTabSelection {
  [self.btnTrend setTitleColor:[UIColor whiteColor]
                      forState:UIControlStateNormal];
  [self.btnIncident setTitleColor:[UIColor whiteColor]
                         forState:UIControlStateNormal];
}

- (void)resetTimeSelection {
  [self.btnAllTime setTitleColor:[UIColor whiteColor]
                        forState:UIControlStateNormal];
  [self.btn1Hour setTitleColor:[UIColor whiteColor]
                      forState:UIControlStateNormal];
  [self.btn24Hours setTitleColor:[UIColor whiteColor]
                        forState:UIControlStateNormal];
  [self.btn7Days setTitleColor:[UIColor whiteColor]
                      forState:UIControlStateNormal];
  [self.btn30Days setTitleColor:[UIColor whiteColor]
                       forState:UIControlStateNormal];
}

- (void)resetIncidentSelection {
  [self.btnIncidentAllTime setTitleColor:[UIColor whiteColor]
                                forState:UIControlStateNormal];
  [self.btnIncident1Hour setTitleColor:[UIColor whiteColor]
                              forState:UIControlStateNormal];
  [self.btnIncident24Hour setTitleColor:[UIColor whiteColor]
                               forState:UIControlStateNormal];
  [self.btnIncident7Day setTitleColor:[UIColor whiteColor]
                             forState:UIControlStateNormal];
  [self.btnIncident30Day setTitleColor:[UIColor whiteColor]
                              forState:UIControlStateNormal];
}
...
- (NSString *)getFormattedString:(int)seconds {
  ...
  NSString *from = [dateFormatter
      stringFromDate:[[NSDate date] dateByAddingTimeInterval:seconds]];
  return from;
}
...
- (void)loadMetricsData:(NSString *)requestURL {
  ...
  [[Global afManager] GET:encodedURL
      ...
      success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        NSString *symbol = @"%";
        self.lblUptime.text = [NSString
            stringWithFormat:@"uptime: %.2f%@",
                             [responseObject[@"uptime"] floatValue], symbol];
        self.lblAvgTime.text = [NSString
            stringWithFormat:@"%.0f ms",
                             [responseObject[@"average"] floatValue] / 1000000];
        self.lblDrops.text =
            [responseObject[@"incident_type_counts"][@"timeout"] stringValue];
        self.lblSlows.text =
            [responseObject[@"incident_type_counts"][@"slow"] stringValue];
        ...
        for (int i = 0; i < array.count; i++) {
          [self.arrayOfValues
              addObject:@([array[i][@"value"] intValue] / 1000000)];
      ...
      success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (self.tableView.frame.size.height <
          ...
          self.scrollView.contentSize = CGSizeMake(
              self.scrollView.frame.size.width, 104 * incidents.count);
        }
...
- (IBAction)onTrend:(id)sender {
  ...
  self.tabSelectionBar.frame = CGRectMake(
      self.tabSelectionBar.frame.origin.x, self.tabSelectionBar.frame.origin.y,
      self.btnTrend.frame.size.width - 10,
      self.tabSelectionBar.frame.size.height);
  ...
  [UIView animateWithDuration:.3f
                   animations:^{
                     self.tabSelectionBar.center = CGPointMake(
                         self.btnTrend.center.x, self.tabSelectionBar.center.y);
                   }];
...
- (IBAction)on1Hour:(id)sender {
  ...
  NSString *requestURL =
      [NSString stringWithFormat:@"/v1/alarms/%d/"
                                 @"response-times?limit=100&from=%@&order_by="
                                 @"timestamp desc",
                                 self.alarm.ID, from];
  [SVProgressHUD showWithStatus:@"Loading" maskType:SVProgressHUDMaskTypeClear];
  [self loadMetricsData:requestURL];
  ...
  [UIView animateWithDuration:.3f
      animations:^{
        self.btnSelectionbar.center =
            CGPointMake(self.btn1Hour.center.x, self.btnSelectionbar.center.y);
      }
      completion:^(BOOL finished) {
...
- (IBAction)on24Hour:(id)sender {
  ...
  NSString *requestURL =
      [NSString stringWithFormat:@"/v1/alarms/%d/"
                                 @"response-times?limit=100&date_trunc=hour&"
                                 @"from=%@&order_by=timestamp desc",
                                 self.alarm.ID, from];
  [SVProgressHUD showWithStatus:@"Loading" maskType:SVProgressHUDMaskTypeClear];
  [self loadMetricsData:requestURL];
  ...
  [UIView animateWithDuration:.3f
      animations:^{
        self.btnSelectionbar.center = CGPointMake(
            self.btn24Hours.center.x, self.btnSelectionbar.center.y);
      }
      completion:^(BOOL finished) {
        [self.btn24Hours setTitleColor:GREEN_COLOR
                              forState:UIControlStateNormal];
      }];
}
...
- (IBAction)on7Days:(id)sender {
  ...
  NSString *requestURL =
      [NSString stringWithFormat:@"/v1/alarms/%d/"
                                 @"response-times?limit=100&date_trunc=day&"
                                 @"from=%@&order_by=timestamp desc",
                                 self.alarm.ID, from];
  [SVProgressHUD showWithStatus:@"Loading" maskType:SVProgressHUDMaskTypeClear];
  [self loadMetricsData:requestURL];
  ...
  [UIView animateWithDuration:.3f
      animations:^{
        self.btnSelectionbar.center =
            CGPointMake(self.btn7Days.center.x, self.btnSelectionbar.center.y);
      }
      completion:^(BOOL finished) {
...
- (IBAction)on30Days:(id)sender {
  ...
  NSString *requestURL = [NSString
      stringWithFormat:
          @"/v1/alarms/%d/response-times?limit=100&date_trunc=day&from=%@",
          self.alarm.ID, from];
  [SVProgressHUD showWithStatus:@"Loading" maskType:SVProgressHUDMaskTypeClear];
  [self loadMetricsData:requestURL];
  ...
  [UIView animateWithDuration:.3f
      animations:^{
        self.btnSelectionbar.center =
            CGPointMake(self.btn30Days.center.x, self.btnSelectionbar.center.y);
      }
      completion:^(BOOL finished) {
        [self.btn30Days setTitleColor:GREEN_COLOR
                             forState:UIControlStateNormal];
      }];
}
...
- (IBAction)onIncident:(id)sender {
  ...
  self.tabSelectionBar.frame =
      CGRectMake(0, self.tabSelectionBar.frame.origin.y,
                 self.btnIncident.frame.size.width - 6,
                 self.tabSelectionBar.frame.size.height);
  ...
  [UIView animateWithDuration:.3f
                   animations:^{
                     self.tabSelectionBar.center =
                         CGPointMake(self.btnIncident.center.x,
                                     self.tabSelectionBar.center.y);
                   }];
}
...
- (IBAction)onIncidentAllTime:(id)sender {
  ...
  [self.btnIncidentAllTime setTitleColor:[UIColor blackColor]
                                forState:UIControlStateNormal];
  [UIView animateWithDuration:.3f
                   animations:^{
                     self.incidentSelectionBar.center =
                         CGPointMake(self.btnIncidentAllTime.center.x,
                                     self.incidentSelectionBar.center.y);
                   }];
}
...
- (IBAction)onIncident1Hour:(id)sender {
  NSString *from = [self getFormattedString:-3600];
  NSString *requestURL = [NSString
      stringWithFormat:@"/v1/alarms/%d/incidents?from=%@&order_by=id desc",
                       self.alarm.ID, from];
  [SVProgressHUD showWithStatus:@"Loading" maskType:SVProgressHUDMaskTypeClear];
  [incidents removeAllObjects];
  [self loadIncidents:requestURL];
  ...
  [self.btnIncident1Hour setTitleColor:[UIColor blackColor]
                              forState:UIControlStateNormal];
  [UIView animateWithDuration:.3f
                   animations:^{
                     self.incidentSelectionBar.center =
                         CGPointMake(self.btnIncident1Hour.center.x,
                                     self.incidentSelectionBar.center.y);
                   }];
}
...
- (IBAction)onIncident24Hour:(id)sender {
  ...
  [self.btnIncident24Hour setTitleColor:[UIColor blackColor]
                               forState:UIControlStateNormal];
  [UIView animateWithDuration:.3f
                   animations:^{
                     self.incidentSelectionBar.center =
                         CGPointMake(self.btnIncident24Hour.center.x,
                                     self.incidentSelectionBar.center.y);
                   }];
}
...
- (IBAction)onIncident7Day:(id)sender {
  ...
  [self.btnIncident7Day setTitleColor:[UIColor blackColor]
                             forState:UIControlStateNormal];
  [UIView animateWithDuration:.3f
                   animations:^{
                     self.incidentSelectionBar.center =
                         CGPointMake(self.btnIncident7Day.center.x,
                                     self.incidentSelectionBar.center.y);
                   }];
}
...
- (IBAction)onIncident30Day:(id)sender {
  ...
  [self.btnIncident30Day setTitleColor:[UIColor blackColor]
                              forState:UIControlStateNormal];
  [UIView animateWithDuration:.3f
                   animations:^{
                     self.incidentSelectionBar.center =
                         CGPointMake(self.btnIncident30Day.center.x,
                                     self.incidentSelectionBar.center.y);
                   }];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  IncidentTableViewCell *cell = (IncidentTableViewCell *)[tableView
      dequeueReusableCellWithIdentifier:@"incident"];
  ...
  [cell.btnView addTarget:self
                   action:@selector(onViewIncident:)
...
- (void)onViewIncident:(id)sender {
  ...
  self.lblOccurredAt.text =
      [self getConvertedStringForIncidentDetails:incident.created_at];
  self.lblResolvedAt.text =
      [self getConvertedStringForIncidentDetails:incident.resolved_at];
