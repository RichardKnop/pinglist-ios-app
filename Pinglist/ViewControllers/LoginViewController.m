...
- (void)viewDidLoad {
  ...
  if (![[Global sharedInstance].accessToken isEqualToString:@""] &&
      [Global sharedInstance].accessToken != nil) {
    [self loadLocations];
    [self goMainScreen];
  }
}
...
- (void)loadLocations {
  ...
  NSString *requestURL =
      [NSString stringWithFormat:@"%@/v1/regions", endpointURL];

  [[Global afManager] GET:requestURL
      parameters:nil
  ...
  if (![[Global sharedInstance].deviceToken isEqualToString:@""] &&
      [Global sharedInstance].deviceToken != nil) {
    NSString *requestURL =
...
- (IBAction)onLogin:(id)sender {
  ...
  [OAuth2Manager authenticateUsingOAuthWithURLString:requestURL
      ...
      success:^(AFOAuthCredential *_Nonnull credential) {
        ...
        // Load locations & go to the main screen
        [self loadLocations];
        [self goMainScreen];
                       ...
                       handler:^(FBSDKLoginManagerLoginResult *result,
                         ...
                         if (error) {
                           ...
                           if ([FBSDKAccessToken currentAccessToken]) {
                             ...
                             [SVProgressHUD
                                 showWithStatus:@"Log in"
                                       maskType:SVProgressHUDMaskTypeClear];
                             NSString *requestURL = [NSString
                                 stringWithFormat:@"%@/v1/facebook/login",
                                                  endpointURL];

                             AFOAuth2Manager *OAuth2Manager = [
                                 [AFOAuth2Manager alloc]
                             ...
                             [OAuth2Manager
                                 authenticateUsingOAuthWithURLString:requestURL
                                 parameters:params
                                 success:^(
                                     AFOAuthCredential *_Nonnull credential) {
                                   [SVProgressHUD dismiss];
                                   ...
                                   [defaults setObject:credential.accessToken
                                                forKey:@"accessToken"];
                                   [defaults setObject:credential.refreshToken
                                                forKey:@"refreshToken"];
                                   [defaults synchronize];
                                           ...
                                           (NSData *)error.userInfo
                                               [AFNetworkingOperationFailingURLResponseDataErrorKey]
                                                  options:kNilOptions
                                                    error:&error];
                                   [SVProgressHUD
                                       showErrorWithStatus:respone[@"error"]
                                                  maskType:
