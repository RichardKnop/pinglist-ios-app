...
- (void)viewDidLoad {
    ...
    self.txtFieldMaxResponseTime.font   =   [UIFont fontWithName:@"Lato-Regular" size:14.0f];
    self.txtFieldUrl.font               =   [UIFont fontWithName:@"Lato-Regular" size:14.0f];
    self.btnLocation.titleLabel.font    =   [UIFont fontWithName:@"Lato-Regular" size:14.0f];
    
    self.txtFieldHttpCode.text          =   [NSString stringWithFormat:@"%d", self.alarm.expected_http_code];
    self.txtFieldUrl.text               =   self.alarm.endpoint_url;
    self.txtFieldInterval.text          =   [NSString stringWithFormat:@"%d", self.alarm.interval];
    self.txtFieldMaxResponseTime.text   =   [NSString stringWithFormat:@"%d", self.alarm.max_response_time];
    ...
    for (int i = 0; i < [Global sharedInstance].locations.count; i ++) {
        ...
        if ([loc.location_id isEqualToString:self.alarm.region]) {
            ...
            [self.btnLocation setTitle:loc.location_name forState:UIControlStateNormal];
        }
    }
    ...
    if (isEmail)
        [self.btnEmail setBackgroundImage:[UIImage imageNamed:@"check"] forState:UIControlStateNormal];
    else
        [self.btnEmail setBackgroundImage:nil forState:UIControlStateNormal];
    
    if (isPushNotification)
        [self.btnPushNotification setBackgroundImage:[UIImage imageNamed:@"check"] forState:UIControlStateNormal];
    else
        [self.btnPushNotification setBackgroundImage:nil forState:UIControlStateNormal];
    
    if (isActive)
        [self.btnActive setBackgroundImage:[UIImage imageNamed:@"check"] forState:UIControlStateNormal];
    else
...
- (IBAction)onUpdate:(id)sender {
    if (![Global validateInteger:self.txtFieldInterval.text]) {
        [Global showAlert:@"Pleae enter the Interval value" sender:self];
        return;
    }
    
    if (![Global validateInteger:self.txtFieldMaxResponseTime.text]) {
        [Global showAlert:@"Pleae enter the Max Response Time" sender:self];
        return;
    }
    
    if (![Global validateInteger:self.txtFieldHttpCode.text]) {
        [Global showAlert:@"Pleae enter the Http Code" sender:self];
        return;
    }
    
    if (self.txtFieldUrl.text.length == 0) {
        [Global showAlert:@"Please enter the URL" sender:self];
        return;
    }
    
    if ([locationName isEqualToString:@"Location"]) {
        [Global showAlert:@"Please select the location" sender:self];
        return;
    }
    
    if (![Global validateUrl:self.txtFieldUrl.text]) {
        [Global showAlert:@"Please enter the valid URL" sender:self];
        return;
    }
    ...
    NSString *requestURL    =   [NSString stringWithFormat:@"%@/v1/alarms/%d", endpointURL, self.alarm.ID];
    NSDictionary *params    =   @{@"region"                    :      locationId,
                                  @"endpoint_url"              :      url,
...
- (IBAction)onLocation:(id)sender {
    ...
    if (!isDropDown) {
        dropDownLocation = [[DropDownListView alloc] initWithTitle:nil options:locations xy:CGPointMake(self.btnLocation.frame.origin.x - 10, self.btnLocation.frame.origin.y + self.btnLocation.frame.size.height - 3) size:CGSizeMake(self.btnLocation.frame.size.width + 34, 150) isMultiple:NO];
...
- (void)DropDownListView:(DropDownListView *)dropdownListView didSelectedIndex:(NSInteger)anIndex array:(NSArray *)data {
    isDropDown      =   NO;
    locationName    =   data[anIndex];
    locationId      =   [(Location *)[Global sharedInstance].locations[anIndex] location_id];
