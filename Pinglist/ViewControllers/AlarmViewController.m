...
- (void)viewDidLoad {
    ...
    NSString    *requestURL     =   [NSString stringWithFormat:@"/v1/alarms?limit=%d&order_by=id desc", pagingLimit];
    [SVProgressHUD showWithStatus:@"Loading" maskType:SVProgressHUDMaskTypeClear];
    [self loadAlarm:requestURL];
...
- (void)loadAlarm :(NSString *)url {
    ...
    if ([Global sharedInstance].credential.isExpired) {
        ...
        NSString *requestURL    =  [NSString stringWithFormat:@"%@%@", endpointURL, url];
        ...
        [[Global afManager] GET:encodedURL parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            self.tableView.frame = CGRectMake(self.tableView.frame.origin.x, 0, self.tableView.frame.size.width, 145 * alarms.count);
            if (self.tableView.frame.size.height < self.scrollView.frame.size.height) {
                ...
                self.scrollView.contentSize = CGSizeMake(self.scrollView.frame.size.width, 145 * alarms.count);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    AlarmTableViewCell *cell        =   (AlarmTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"alarm_cell"];
    ...
    cell.lblHttpCode.text           =       [NSString stringWithFormat:@"%d", alarm.expected_http_code];
    ...
    if (cell.isInactive) {
        ...
        [cell.btnSwitch setImage:[UIImage imageNamed:@"off"] forState:UIControlStateNormal];
    }
    ...
    [cell.btnSwitch addTarget:self action:@selector(onSwitch:) forControlEvents:UIControlEventTouchUpInside];
    [cell.btnEdit addTarget:self action:@selector(onEdit:) forControlEvents:UIControlEventTouchUpInside];
    [cell.btnDelete addTarget:self action:@selector(onDelete:) forControlEvents:UIControlEventTouchUpInside];
    [cell.btnPerformance addTarget:self action:@selector(onPerformance:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)onSwitch :(id)sender {
    ...
    AlarmTableViewCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:[sender tag] inSection:0]];
    if (cell.isLocked) {
        return;
    }
    ...
    if (!cell.isInactive) {
        ...
        [cell.btnSwitch setImage:[UIImage imageNamed:@"on"] forState:UIControlStateNormal];
    ...
    if ([Global sharedInstance].credential.isExpired) {
        [Global refreshAccessToken:^(NSString *accessTok, NSString *refreshTok) {
            [Global sharedInstance].accessToken = accessTok;
            [Global sharedInstance].refreshToken = refreshTok;
        ...
        NSString *requestURL    =   [NSString stringWithFormat:@"%@/v1/alarms/%d", endpointURL, selectedAlarm.ID];
        NSDictionary *params    =   @{@"region"                    :      selectedAlarm.region,
                                      @"endpoint_url"              :      selectedAlarm.endpoint_url,
                                      @"expected_http_code"        :      [NSNumber numberWithInt:selectedAlarm.expected_http_code],
                                      @"max_response_time"         :      [NSNumber numberWithInt:selectedAlarm.max_response_time],
                                      @"interval"                  :      [NSNumber numberWithInt:selectedAlarm.interval],
                                      @"email_alerts"              :      [NSNumber numberWithBool:selectedAlarm.isEmailAlert],
                                      @"push_notification_alerts"  :      [NSNumber numberWithBool:selectedAlarm.isPushNotification],
...
- (void)delete :(Alarm *)alarm{
    if ([Global sharedInstance].credential.isExpired) {
        ...
        NSString *requestURL    =   [NSString stringWithFormat:@"%@/v1/alarms/%d", endpointURL,alarm.ID];
        [SVProgressHUD showWithStatus:@"Deleting" maskType:SVProgressHUDMaskTypeClear];
        
        [[Global afManager] DELETE:requestURL parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
            [SVProgressHUD showSuccessWithStatus:@"Done"];
