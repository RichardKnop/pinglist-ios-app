...
- (void)viewDidLoad {
    ...
    //Load teams
    NSString *requestURL    =  [NSString stringWithFormat:@"%@/v1/teams?order_by=id desc", endpointURL];
...
- (void)loadTeam :(NSString *)url {
    if ([Global sharedInstance].credential.isExpired) {
        ...
        [[Global afManager] GET:encodedURL parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            self.tableView.frame = CGRectMake(self.tableView.frame.origin.x, 0, self.tableView.frame.size.width, 95 * teams.count);
            if (self.tableView.frame.size.height < self.scrollView.frame.size.height) {
                ...
                self.scrollView.contentSize = CGSizeMake(self.scrollView.frame.size.width, 95 * teams.count);
...
- (void)delete :(Team *)t {
    if ([Global sharedInstance].credential.isExpired) {
        ...
        NSString *requestURL   =   [NSString stringWithFormat:@"%@/v1/teams/%d", endpointURL,t.ID];
        ...
        [[Global afManager] DELETE:requestURL parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            [self.tableView reloadData];
            self.tableView.frame        =   CGRectMake(0, 0, self.tableView.frame.size.width, 95 * teams.count);
            self.scrollView.contentSize =   CGSizeMake(self.scrollView.frame.size.width, 95 * teams.count);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    TeamTableViewCell *cell         =   (TeamTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"team"];
