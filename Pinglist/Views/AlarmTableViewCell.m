...
- (IBAction)onLock:(id)sender {
  if (self.isLocked)
    [self.btnLock setImage:[UIImage imageNamed:@"unlocked"]
                  forState:UIControlStateNormal];

  else
    [self.btnLock setImage:[UIImage imageNamed:@"locked"]
                  forState:UIControlStateNormal];
...
- (IBAction)onArrow:(id)sender {
  if (!self.isExpanded) {
    ...
    [UIView animateWithDuration:.2f
        animations:^{
          self.vwBackground.frame =
              CGRectMake(0, self.vwBackground.frame.origin.y,
                         self.vwBackground.frame.size.width,
                         self.vwBackground.frame.size.height);
        }
        completion:^(BOOL finished) {
          [self.btnArrow setImage:[UIImage imageNamed:@"left_arrow"]
                         forState:UIControlStateNormal];
        }];
  }
    ...
    [UIView animateWithDuration:.2f
        animations:^{
          self.vwBackground.frame =
              CGRectMake(-50, self.vwBackground.frame.origin.y,
                         self.vwBackground.frame.size.width,
                         self.vwBackground.frame.size.height);
        }
        completion:^(BOOL finished) {
          [self.btnArrow setImage:[UIImage imageNamed:@"right_arrow"]
                         forState:UIControlStateNormal];
        }];
  }
